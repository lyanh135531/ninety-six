            ...
            <div className="bg-white/95 backdrop-blur-md rounded-2xl p-3 shadow-2xl border border-white/20">
              {hasSizes ? (
                  ...
                  <div className="flex flex-wrap justify-center gap-1.5">
                    {availableSizes.map(size => (
                      <button
                        ...
                        className="min-w-[40px] h-9 flex items-center justify-center rounded-xl border border-gray-100 text-xs font-bold text-gray-700 hover:bg-teal-700 hover:border-teal-700 hover:text-white transition-all cursor-pointer disabled:opacity-50"
                      >
                        {size}
                      </button>
                    ))}
                ...
                <button 
                  ...
                  disabled={isAdding}
                  className="w-full py-3 bg-teal-700 text-white font-black text-xs rounded-xl shadow-lg hover:bg-teal-800 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-80"
                >
        ...
        <div className="p-5 flex-1 flex flex-col">
          ...
          <div className="mt-4 pt-4 border-t border-gray-50 hidden md:flex items-center justify-between">
             <button 
               onClick={handleQuickAdd}
               disabled={isAdding}
               className="text-[11px] font-black text-teal-700 hover:underline flex items-center gap-1.5 cursor-pointer uppercase tracking-wider disabled:opacity-50"
             >
               <ShoppingBag className="w-3.5 h-3.5" /> {hasSizes ? "Chọn Size" : "Thêm vào giỏ"}
             </button>
          ...
          <div className="mt-4 md:hidden">
            <button 
              onClick={handleQuickAdd}
              disabled={isAdding}
              className="w-full py-3.5 bg-teal-700 text-white font-black text-xs rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-80"
            >
              {hasSizes ? (
                <>CHỌN KÍCH CỠ</>
              ) : (
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              {availableSizes.map(size => (
                <button
                  ...
                  className="h-12 rounded-2xl font-bold border-2 border-gray-100 text-gray-500 hover:border-teal-700 hover:bg-teal-50 hover:text-teal-700 transition-all flex items-center justify-center cursor-pointer active:scale-95 disabled:opacity-50"
                >
                  {size}
                </button>
              ))}
