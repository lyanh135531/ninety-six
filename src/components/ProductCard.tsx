...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const theme = {
    ...
    featuredBadge: isBaby ? "bg-rose-800" : "bg-teal-700",
    ...
    title: isBaby ? "text-rose-800" : "text-teal-900",
    badge: isBaby
      ? "text-rose-800/80 bg-rose-50/50 border-rose-100/50"
      : "text-teal-800/40 bg-gray-50 border-gray-100",
    textMuted: isBaby ? "text-rose-800/60" : "text-teal-800/40",
    textSecondary: isBaby ? "text-rose-900" : "text-teal-900",
    iconMuted: isBaby ? "text-rose-800/40" : "text-teal-800/60",
  };
  ...
  return (
    <>
      {/* ── Card ── */}
      <article
        className="group relative bg-white rounded-[1.75rem] overflow-hidden flex flex-col h-full transition-all duration-500 hover:-translate-y-2"
        style={{
      ...
      >
        ...
        <Link
          ...
          className="relative aspect-[3/4] overflow-hidden bg-gray-50 shrink-0 block cursor-pointer"
          tabIndex={-1}
        >
          {product.imageUrl ? (
            <Image
          ...
          <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5">
            {product.isFeatured && (
              <span
                className={`inline-flex items-center gap-1 ${theme.featuredBadge} text-white text-[9px] font-black px-2.5 py-1 rounded-full shadow-md`}
              >
            ...
            {isNew && (
              <span
                className={`inline-flex items-center gap-1 ${theme.newBadge} text-white text-[9px] font-black px-2.5 py-1 rounded-full shadow-md`}
              >
                ✨ MỚI
              </span>
            )}
          ...
          <div
            className={`absolute top-3 right-3 z-10 w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center transition-all duration-200 opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 cursor-pointer hidden md:flex ${isBaby ? "text-rose-400 hover:text-white hover:bg-rose-600" : "text-teal-800/60 hover:text-white hover:bg-teal-700"}`}
          >
            ...
            <div className="bg-white/95 backdrop-blur-md rounded-2xl p-3 shadow-2xl border border-white/50">
              {hasSizes ? (
                <div className="space-y-2">
                  <p className={`text-[9px] font-black text-center uppercase tracking-[0.2em] ${theme.textMuted}`}>
                    Chọn kích cỡ
                  </p>
                        ...
                        <button
                          ...
                          className={`relative min-w-[36px] h-8 text-xs font-bold rounded-xl border transition-all duration-200 cursor-pointer disabled:opacity-50 ${
                            addedSize === size
                              ...
                              : out
                                ? isBaby
                                  ? "bg-rose-50/50 border-rose-100/50 text-rose-800/40 cursor-not-allowed"
                                  : "bg-gray-50 border-gray-100 text-teal-800/30 cursor-not-allowed"
                                : `bg-white border-gray-200 hover:text-white hover:scale-105 ${
                                    isBaby
                                      ? "text-rose-800 hover:bg-rose-600 hover:border-rose-600"
                                      : "text-teal-900 hover:bg-teal-700 hover:border-teal-700"
                                  }`
                          }`}
                        >
                          {size}
                ...
                >
                  <ShoppingBag
                    className={`w-3.5 h-3.5 ${isAdding ? "animate-bounce" : ""}`}
                  />
                  {isAdding
                    ? "ĐANG THÊM..."
                    : isOutOfStock
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          >
            <span
              className={`inline-block text-[9px] font-black tracking-[0.18em] uppercase px-2 py-0.5 rounded-md ${isBaby ? "text-rose-800/80 bg-rose-50/50" : "text-teal-900 bg-teal-50"}`}
            >
              {product.category.name}
            </span>
            <h3
              className={`mt-2 ${theme.title} font-bold text-sm leading-tight line-clamp-2 transition-colors duration-200 ${theme.hoverText}`}
            >
              {product.name}
            </h3>
            <div className="mt-2.5 flex items-center gap-2">
              <p className={`text-lg font-black ${isBaby ? "text-rose-800" : "text-teal-900"}`}>
                {formatCurrency(product.price)}
              </p>
              {hasSizes && (
                <span
                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded border whitespace-nowrap shrink-0 ${theme.badge}`}
                >
      ...
      {isModalOpen && (
        ...
        >
          ...
          <div className="animate-scale-in relative bg-white rounded-[2rem] p-6 sm:p-8 max-w-sm w-full shadow-2xl border border-gray-100">
            ...
            >
              <X className={`w-4 h-4 ${theme.iconMuted}`} />
            </button>
            ...
            <div className="flex gap-4 mb-6 items-center">
              ...
              <div>
                <p
                  className={`text-[10px] font-black uppercase tracking-widest ${theme.primary}`}
                >
                  Chọn kích thước
                </p>
                <h4 className={`font-bold line-clamp-2 leading-tight text-sm mt-0.5 ${theme.textSecondary}`}>
                  {product.name}
                </h4>
                <p className={`${theme.primary} font-black text-base mt-1`}>
                  {formatCurrency(product.price)}
                </p>
              </div>
            </div>
                  ...
                  <button
                    ...
                    className={`relative h-12 rounded-2xl font-bold border-2 text-sm transition-all active:scale-95 cursor-pointer disabled:opacity-50 ${
                      out
                        ...
                        : `border-gray-200 ${
                            isBaby
                              ? "text-rose-800 hover:border-rose-600 hover:bg-rose-50 hover:text-rose-800"
                              : "text-teal-800 hover:border-teal-700 hover:bg-teal-50 hover:text-teal-900"
                          }`
                    }`}
                  >
                    {size}
            ...
            <p className={`text-center text-[10px] font-bold uppercase tracking-[0.2em] ${isBaby ? "text-rose-400" : "text-teal-800/30"}`}>
